...
# 2026-08-24 확장분(21개 품목)은 표준 4종 쿼리로 자동 채웁니다. '김장 배추'
# 처럼 품목 특유의 유행어를 더 넣고 싶으면 위 딕셔너리에 직접 추가하십시오.
for _item in config.ITEMS:
    QUERIES.setdefault(_item, [f"{_item} 가격", f"{_item} 도매가",
                               f"{_item} 출하", f"{_item} 작황"])
del _item
...
#
# 2026-08-24 실측 튜닝: 실제 수집분(3,028건)의 "기타" 비중이 63%(1,922건)여서
# 표본을 직접 읽고 보강했습니다. 가장 큰 원인은 원인 불명시 가격등락
# 헤드라인("배춧값 75% 급등" 류)이 어떤 규칙에도 안 걸리는 것이었습니다 —
# "급등/폭락" 등은 강도(intensity) 판정에만 쓰이고 카테고리 분류에는
# 전혀 반영되지 않고 있었습니다. 마지막 우선순위로 "가격변동"을 추가해
# 원인이 특정되지 않는 단순 등락 기사를 잡습니다(방향은 기존 강도/방향
# 폴백 로직이 처리).
CATEGORY_RULES: list[tuple[str, list[str]]] = [
    ("기상피해", ["폭우", "호우", "장마", "태풍", "가뭄", "한파", "폭염", "냉해",
               "우박", "서리", "일조", "침수", "이상기후", "고온"]),
    ("작황부진", ["작황", "생육", "병해충", "무름병", "바이러스", "흉작", "품질 저하",
               "생산량 감소", "생산 감소", "생산 줄어", "재배면적", "재배지 면적",
               "면적 감소"]),
    ...
    # 원인은 특정되지 않지만 명백한 가격 등락 신호. 위 카테고리에 안 걸린
    # 경우에만 마지막으로 검사합니다.
    ("가격변동", ["급등", "폭등", "급락", "치솟", "고공행진", "금값",
               "금추", "금파", "금양파"]),
]
...
def _is_relevant(title: str, item: str) -> bool:
    """제목에 품목명이 실제로 등장하는지 확인.

    네이버 뉴스 검색은 제목이 아니라 본문/설명까지 포함해 매칭하므로,
    쿼리에 없는 품목이 헤드라인인 기사가 대량으로 섞여 들어옵니다.
    실측(2026-08-24)으로 확인한 규모: item='무' 쿼리 결과 중 제목에
    '무'가 있는 것은 19%뿐이고, 나머지 81%는 그 시기 가장 큰 채소값
    사건(주로 배추)의 기사였습니다 — '대파' 쿼리는 92%가 이런 식으로
    다른 품목 기사였습니다. 이렇게 되면 대파의 뉴스 피처가 실제로는
    배추 사건 신호가 되어, 품목별 뉴스 신호가 서로 뒤섞입니다.

    단순 부분일치를 씁니다. '무'처럼 한 글자짜리 품목명은 다른 단어에
    포함될 위험이 있지만(예: '휴무일'), 실측 확인 결과 '무값'·'무밭'·
    '저장무'처럼 조사·복합어가 직접 붙는 경우가 압도적으로 많아 경계
    정규식을 쓰면 오히려 정상적인 매치까지 막습니다. 단어 경계를
    엄격히 따지려면 형태소 분석기가 필요한데, 이 프로젝트 규모에서는
    과합니다. 대신 대시보드에서 실제로 눈에 띈(2026-08-24) '무주반딧불
    축제'·'무조건' 같은 흔한 오탐만 짧은 예외 목록으로 걸러냅니다.
    """
    if item == "무":
        for bad in _MU_FALSE_POSITIVES:
            title = title.replace(bad, "")
    return item in title


# '무'와 함께 흔히 쓰여 오탐을 일으키는 단어. 형태소 분석기 없이 흔한
# 사례만 손으로 걸러낸 목록이라 완전하지 않습니다 — 필요할 때 추가하십시오.
_MU_FALSE_POSITIVES = [
    "무주", "무조건", "무료", "무역", "무기", "무시", "무한", "무능",
    "무비자", "무상", "무산", "휴무", "공무", "사무", "업무", "전무",
    "상무", "총무", "의무", "무대", "무늬", "무사히", "무려", "무엇",
    "마켓무버", "무버",
]
...
def fetch_news(items: list[str] | None = None, pages: int = 3,
        ...
        for query in QUERIES.get(item, [f"{item} 가격"]):
            for page in range(pages):
                ...
                for it in got:
                    ...
                    if not title:
                        continue
                    if not _is_relevant(title, item):
                        irrelevant += 1
                        continue
                    try:
    ...
    print(f"  제목에 품목명이 없어 제외: {irrelevant:,}건")
    if not rows:
